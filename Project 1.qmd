...
```{r}
#Creating a wrapper function to read in data and perform data processing steps 1-6
wrapper_function <- function(url, value = "values for enrollment") {
  ...
  tibbles <- read_csv(url) |>
    ...
    #Using the Steps 4-6 function to divide into two tibbles and add variables and class
...
plot.state <- function(state_tibble, var_name="value") {
  ...
  #Returning a plot that displays mean enrollment over the years for each division
...
plot.county <- function(county_data, State = "KY", top_or_bottom = "top",
  ...
  mean_tibble <- county_data |>
    #Selecting only rows with a state value matching the user provided value or default
...
#Using the short combining function to combine the two tibbles produced by the wrapper
